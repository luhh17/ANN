...
class Model:
    def __init__(self,
        ...
        # self.gamma = tf.Variable(initial_value=1.0, trainable=True)
        # self.beta = tf.Variable(initial_value=0.1, trainable=True)
        # self.train_mean = tf.Variable()
        # self.train_var = tf.Variable()
    ...
    def forward(self, is_train, reuse=None):
        with tf.variable_scope("model", reuse=reuse):
            ...
            # Your Linear Layer
            temp = tf.layers.dense(inputs=self.x_, units=512)
            temp = batch_normalization_layer(incoming=temp, is_train=is_train)
            temp = tf.nn.relu(temp)
            temp = dropout_layer(incoming=temp, drop_rate=0.4, is_train=is_train)
